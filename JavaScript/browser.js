...
async function showEngineView(target, wisp = settings.wispUrl) {
    ...
    const onUrlChange = (currentUrl) => {
        const tab = activeTab();
        if (!tab || !currentUrl) return;
        tab.url = currentUrl;
        tab.title = new URL(currentUrl).hostname;
        el.address.value = currentUrl;
        renderTabs();
    };

    if (adapter.launch) {
        await adapter.launch({ target, wisp, frame: el.frame, onUrlChange });
        el.content.hidden = false;
    } else if (adapter.getUrl) {
        const result = await adapter.getUrl({ target, wisp });
...
function moveHistory(delta) {
    const adapter = settings.engine === "scramjet" ? window.owoScramjetAdapter : null;
    if (adapter && delta < 0 && adapter.back) { void adapter.back(); return; }
    if (adapter && delta > 0 && adapter.forward) { void adapter.forward(); return; }
    const tab = activeTab();
...
$("#reload").addEventListener("click", async () => {
    ...
    try {
        const adapter = settings.engine === "scramjet"
            ? window.owoScramjetAdapter
            : window.owoUltravioletAdapter;
        const reloaded = adapter?.reload ? await adapter.reload() : false;
        if (!reloaded) await openCurrentTab();
    } catch (error) {
        showNotice(error instanceof Error ? error.message : String(error));
    } finally {
        el.progress.classList.remove("loading");
